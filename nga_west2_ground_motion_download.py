...
class NGAWest2GroundMotionDownload:
    ...
    def ground_motion_info(self):
        ...
        RSNs = df['Record Sequence Number'].tolist()

        acc_file_names = []
        for direction in self.gm_directions:
            if direction == 'horizontal 1':
                acc_file_name_base = df['File Name (Horizontal 1)']
            elif direction == 'horizontal 2':
                acc_file_name_base = df['File Name (Horizontal 2)']
            elif direction == 'vertical':
                acc_file_name_base = df['File Name (Vertical)']
            else:
                raise ValueError("Invalid direction. Please choose from 'horizontal1', 'horizontal2', or 'vertical'.")

            for rsn, file_name in zip(RSNs, acc_file_name_base):
                acc_file_name = 'RSN' + str(rsn) + '_' + file_name.replace('\\', '_')
                acc_file_names.append(acc_file_name)

        return RSNs, acc_file_names

    def download_ground_motion_file(self, RSNs):
        ...
        # Open NGA West 2 website
        browser.get('https://ngawest2.berkeley.edu/users/sign_in?unauthenticated=true')
    ...
    def organize_acceleration_data(self, acc_file_names):
        """
        This function reads the acceleration data from the downloaded PEER NGA-West2 records and organizes it into a DataFrame.
    ...
    def run_download_script(email: str, password: str, flatfile_name: str, browser_type: str, gm_directions: list):
       ...
       'vertical'
        """
        downloader = NGAWest2GroundMotionDownload(email, password, flatfile_name, browser_type, gm_directions)
        RSNs, acc_file_names = downloader.ground_motion_info()
        downloader.download_ground_motion_file(RSNs)
